...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const truncatedTitle = product.title.length > 15 ? product.title.substring(0, 15) + "..." : product.title;

  return (
    <div className="w-[232px] h-[383px] border border-[#CDD8DF] mt-2 relative product-card bg-white rounded" >
      ...
      <div className="w-[94%] m-auto h-[48px]">
        <div className="w-full h-[22px] flex justify-between items-center text-[16px] font-[500] text-[#388E3C]">
          <div className="flex justify-center items-center gap-2">
            <p>${product.price}</p>
            <div className="border rounded-full px-2 h-[14px] text-[10px] flex justify-center items-center cursor-pointer">ebay</div>
          </div>
